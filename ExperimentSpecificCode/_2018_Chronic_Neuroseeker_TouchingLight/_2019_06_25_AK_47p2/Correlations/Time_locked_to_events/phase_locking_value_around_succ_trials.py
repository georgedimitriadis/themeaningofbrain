...
start_offset = 0* const.SAMPLING_FREQUENCY
trials_start = succesful_trials + start_offset
trials_window_size = 1.0 * const.SAMPLING_FREQUENCY

imf_subsampling_factor = const.LFP_DOWNSAMPLE_FACTOR
...
def spikes_matrix(_start_offset, _trials_window_size):
    _start_offset = _start_offset * const.SAMPLING_FREQUENCY
    _trials_window_size = _trials_window_size * const.SAMPLING_FREQUENCY
    ...
    with open(join(results_folder, 'EventsCorrelations', 'Poke',
                   'spike_binary_matrix_subsampled_between_{}_and_{}_succ_trials.pcl'.
                           format(_start_offset / const.SAMPLING_FREQUENCY,
                                  (_start_offset + _trials_window_size) / const.SAMPLING_FREQUENCY)),
              'wb') as f:
...
# Create spike matrices for many times around the event
...
with open(join(results_folder, 'SpikeLfpCorrelations', 'SpikesAwayFromSuccTrials',
               'spike_binary_matrix_subsampled_between_{}_and_{}_succ_trials.pcl'.
                       format(start_offset / const.SAMPLING_FREQUENCY,
                              (start_offset + trials_window_size) / const.SAMPLING_FREQUENCY)),
          'rb') as f:
...
psd_imf, fs = mt.psd_array_multitaper(imfs[:, :, :20000],
                                      sfreq=const.SAMPLING_FREQUENCY/imf_subsampling_factor,
