...
tsne_folder = join(tsne_folder_base, 'CroppedVideo_100ms_Top100PCs_ZoomedOut')
...
# grayscale_resized_cropped_video_array_name = 'grayscale_resized_cropped_video_array.npy'
grayscale_resized_cropped_video_array_name = 'grayscale_resized_cropped_zoomed_out_video_array.npy'
...
np.save(join(subsumpled_video_folder, grayscale_resized_cropped_video_array_name.npy), grayscale_resized_video_array)


#    Have a look to see if resulting subsampled images are ok
grayscale_resized_video_array = np.load(join(subsumpled_video_folder, grayscale_resized_cropped_video_array_name))
...
grayscale_resized_video_array_frame_smoothed = np.transpose(
    binning.rolling_window_with_step(grayscale_resized_video_array.transpose(),
                                     np.mean, 12, 12))
np.save(join(subsumpled_video_folder, '{}_smoothed.npy'.format(grayscale_resized_cropped_video_array_name[:-4])),
...
pickle.dump(pca_flat_video, open(join(subsumpled_video_folder, 'full_pca_of_zoomedout_greyscale_subsampled_video.pcl'), 'wb'))
np.save(join(subsumpled_video_folder, 'all_pcs_of_zoomedout_greyscale_subsampled_video.npy'), pcs_flat_video)

pca_flat_video = pickle.load(open(join(subsumpled_video_folder, 'full_pca_of_zoomedout_greyscale_subsampled_video.pcl'), 'rb'))
...
pcs_flat_video = np.load(join(subsumpled_video_folder, 'all_pcs_of_zoomedout_greyscale_subsampled_video.npy'))
