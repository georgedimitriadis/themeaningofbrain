...
avg_muae_around_tp, std_muae_around_tp, time_axis = \
    tla_funcs.time_lock_raw_data(ap_data, events, times_to_cut=[-window_time, window_time],
                                 ...
                                 high_pass_cutoff=None, rectify=True, low_pass_cutoff=2000,
                                 avg_reref=True, keep_trials=False)

np.save(join(results_folder, 'Lfp', 'Averages', 'Muaes_around_tp_2Kto7K.npy'), avg_muae_around_tp)
np.save(join(results_folder, 'Lfp', 'Averages', 'Muaes_around_ntp.npy'), avg_muae_around_ntp)


#  Normalise

avg_muae_around_tp = np.load(join(results_folder, 'Lfp', 'Averages', 'Muaes_around_tp_2Kto7K.npy'))
...
tp_n = np.empty((avg_muae_around_tp.shape))
for i in np.arange(len(avg_muae_around_tp)):
    tp_n[i, :] = binning.scale(avg_muae_around_tp[i], 0, 1)
