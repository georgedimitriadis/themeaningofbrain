...
# -------------------------------------------------
# <editor-fold desc="SPIKE TSNE CORRELATED WITH VIDEO TSNE">

video_tsne_labels_file = join(analysis_folder, 'Tsnes', 'Video', 'CroppedVideo_100ms_Top100PCs_ZoomedOut', 'dbscan_labels.npy')
...
unique_labels = np.unique(video_tsne_labels)
unique_labels = np.delete(unique_labels, np.argwhere(unique_labels==-1))


random_selection_of_tsne_points_indices = np.random.choice(np.arange(len(tsne_result)), 20000)
random_selection_of_tsne_points = tsne_result[random_selection_of_tsne_points_indices, :]
mean_distance_of_all = np.median(cdist(random_selection_of_tsne_points, random_selection_of_tsne_points))

mean_distances_of_labels = []
for l in unique_labels:
    tsne_of_label = tsne_result[video_tsne_labels == l]
    mean_distances_of_labels.append(np.median(cdist(tsne_of_label, tsne_of_label)) / mean_distance_of_all)


plt.boxplot(mean_distances_of_labels)
# </editor-fold>
# -------------------------------------------------
