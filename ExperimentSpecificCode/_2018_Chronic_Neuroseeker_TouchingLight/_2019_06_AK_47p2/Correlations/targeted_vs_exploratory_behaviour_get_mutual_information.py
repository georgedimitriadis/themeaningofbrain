...
minimum_distance_of_rat_to_poke_or_ball = 1  # That means 25 cm vertical and 30 cm horizontal
...
frame = 0
ev_video = event_dataframes['ev_video']
starts_of_non_patterned_behaviour = [s[0] for s in long_continous_non_patterned_behaviour_frame_ranges]
end_of_non_patterned_behaviour = [s[-1] for s in long_continous_non_patterned_behaviour_frame_ranges]
...
shuffled, mean, conf_intervals = MI.shuffle_test(MI.mi_LNC,  spike_rates_patterned_behaviour_0p25[max_neuron],
                                                 ...
                                                 z=False, ns=1000, ci=0.95, k=10, base=np.exp(1), alpha=0.4,
                                                 intens=1e-10)
np.save(join(mutual_information_folder, 'shuffled_mut_info_spike_rate_912_vs_distance_to_poke_patterned_behaviour.npy'),
        shuffled)
...
shuffled = np.load(join(mutual_information_folder, 'shuffled_mut_info_spike_rate_912_vs_distance_to_poke_patterned_behaviour.npy'))
...
def draw1(index):
    ...
    bin_fr = binning.rolling_window_with_step(
        spike_rates_patterned_behaviour_0p25[distance_to_poke_corr_neurons_pb_index[index], :], np.mean, 8, 1)
    norm_bin_fr = (bin_fr - np.min(bin_fr)) / (np.max(bin_fr) - np.min(bin_fr))
    plt.plot(norm_bin_fr)
...
def draw2(index):
    plt.clf()
    norm_s = (distance_to_poke_non_patterned_behaviour_0p25 - np.min(distance_to_poke_non_patterned_behaviour_0p25)) / \
    (np.max(distance_to_poke_non_patterned_behaviour_0p25) - np.min(distance_to_poke_non_patterned_behaviour_0p25))
    plt.plot(norm_s)

    bin_fr = binning.rolling_window_with_step(
        spike_rates_non_patterned_behaviour_0p25[distance_to_poke_corr_neurons_non_pb_index[index], :], np.mean, 8, 1)
    norm_bin_fr = (bin_fr - np.min(bin_fr)) / (np.max(bin_fr) - np.min(bin_fr))
    plt.plot(norm_bin_fr)
    plt.vlines(x=np.cumsum(lengths_of_windows_of_non_patterned_behaviour) / 30, ymin=0,
               ymax=1)
    plt.title(str(mi_non_pb_spikes_vs_distance_to_poke[distance_to_poke_corr_neurons_non_pb_index[index]]))
    return None


index = 0
out = None
sl.connect_repl_var(globals(), 'index', 'out', 'draw2', slider_limits=[0, len(distance_to_poke_corr_neurons_non_pb_index) - 1])


np.intersect1d(distance_to_poke_corr_neurons_pb_index, distance_to_poke_corr_neurons_non_pb_index)
print(distance_to_poke_corr_neurons_non_pb_index[index])
