...
#Recordings 2014-10-10 and 2014-10-17 the signal was saved uint16; all channels are pristine
...
#Electronic noise W different resistors there are channels that are not fully connected; remove this channels
...
#Recordings 2014-11-13 and 2014-11-25 the signal was saved uint16; 16 channels pedot and 16 channels pristine
...
#------------------------------------------------------------------------------------------------------------
#Open RMS and Median Noise npy to .txt to Origin
...
#Open RMS and Median Noise in brain
...
#RMS amplitude----------------------------------------------
stdvs_PEDOT = np.median(np.abs(temp_filtered_uV[channels_pedot])/0.6745, axis=1)
stdvs_pristine = np.median(np.abs(temp_filtered_uV[channels_pristine])/0.6745, axis=1)
stdv_PEDOT_average = np.average(stdvs_PEDOT)
stdv_pristine_average = np.average(stdvs_pristine)
stdv_PEDOT_stdv = stats.sem(stdvs_PEDOT)
stdv_pristine_stdv = stats.sem(stdvs_pristine)
print('#------------------------------------------------------')
print('RMS_PEDOT:'+ str(stdvs_PEDOT))
print('RMS_Pristine:'+ str(stdvs_pristine))
print('RMS_PEDOT_average:'+ str(stdv_PEDOT_average))
print('RMS_PEDOT_stdv:'+ str(stdv_PEDOT_stdv))
...
stdv_ch_average = np.average(stdvs_ch[np.where(stdvs_ch < 4)])
#stdv_ch_stdv = stats.sem(stdvs_ch)
stdv_ch_stdv = stats.sem(stdvs_ch[np.where(stdvs_ch < 4)])
