...
#impedance
impedancevalues = np.genfromtxt(r'Z:\j\Joana Neto\Neuroseeker256ch\Type1_Probe6\2017-02-08\impedance-2017-02-08.txt')
...
#impedancevalues_MOhm_norm = np.round(impedancevalues_MOhm_norm_float, 2)
fig = plt.figure()
ax1 = fig.add_subplot(1,2,1)
channel_positions=polytrode_256channels(bad_channels=[-1])
...
#image, scat = plot_topoplot256(ax1, channel_positions,impedancevalues_MOhm, show=False, interpolation_method='none', gridscale=5, zlimits=[np.min(impedancevalues_MOhm_norm), np.max(impedancevalues_MOhm_norm)])
ax1.set_yticks([], minor=False)
ax1.set_xticks([], minor=False)
...
#saline
filename_RMS_saline = r"C:\Users\KAMPFF-LAB-ANALYSIS3\Google Drive\Thesis Chapter\Chapter 4\Pictures\Previous_figure6_ch3_\Noise\saline_amplifier2017-02-07T18_12_22\250noise_Median.npy"
...
channel_positions = polytrode_256channels(bad_channels=[-1])
#image, scat = plot_topoplot256(ax1, channel_positions, RMSvalues_norm, show=False, interpolation_method='none', gridscale=5, zlimits=[np.min(RMSvalues_norm), np.max(RMSvalues_norm)])
image, scat = plot_topoplot256(ax1, channel_positions, RMSvalues_saline, show=False, interpolation_method='none', gridscale=5, zlimits=[np.min(RMSvalues_saline), np.max(RMSvalues_saline)])
ax1.set_yticks([], minor=False)
ax1.set_xticks([], minor=False)
plt.colorbar(mappable=image)
#plt.colorbar(mappable=image, ticks = np.arange(np.min(RMSvalues_norm), np.max(RMSvalues_norm), 0.1))
...
#brain
filename_RMS_brain = r"C:\Users\KAMPFF-LAB-ANALYSIS3\Google Drive\Thesis Chapter\Chapter 4\Pictures\Previous_figure6_ch3_\Noise\brain_amplifier2017-02-08T15_34_04\250noise_Median.npy"
...
filename_RMS_brain =r"Z:\j\Joana Neto\Neuroseeker256ch\Type1_Probe6\2017-02-16\Datakilosort\amplifier2017-02-16T15_37_59\250noise_Median.npy"
