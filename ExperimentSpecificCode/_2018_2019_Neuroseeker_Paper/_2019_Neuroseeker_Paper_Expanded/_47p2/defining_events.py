...
from ExperimentSpecificCode._2018_2019_Neuroseeker_Paper._2019_Neuroseeker_Paper_Expanded._47p2 import constants_47p2 as const
from ExperimentSpecificCode._2018_2019_Neuroseeker_Paper._2019_Neuroseeker_Paper_Expanded import constants_common as const_comm
...
beam_break = event_dataframes['ev_food_beam_break']
...
# In case there are no beam_breaks TTL pulses
successful_trial_pokes = reward_sounds
...
sync_window = 0.5
sync_window_timepoints = int(sync_window * const_comm.SAMPLING_FREQUENCY)
...
minimum_delay = 5
time_to_previous_poke = np.diff(beam_breaks[:, 0])/const_comm.SAMPLING_FREQUENCY
...
for r_sound in successful_trial_pokes:
    t = np.logical_and(first_pokes_after_delay > r_sound - 2 * const_comm.SAMPLING_FREQUENCY,
                       first_pokes_after_delay < r_sound + 2 * const_comm.SAMPLING_FREQUENCY)
    if np.any(t):
...
for tb_event in events_touch_ball:
    ...
    if suc_trial_poke_after_tb_event - tb_event < 40 * const_comm.SAMPLING_FREQUENCY:
