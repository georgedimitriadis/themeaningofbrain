...
date_folder = 8
analysis_folder = join(const_rat.base_save_folder, const_rat.rat_folder, const_rat.date_folders[date_folder],
                       'Analysis', 'NeuropixelSimulations', 'Long')

dlc_folder = join(const_rat.base_save_folder, const_rat.rat_folder, const_rat.date_folders[date_folder],
                       'Analysis', 'Deeplabcut')

dlc_project_folder = join(dlc_folder, 'projects', 'V1--2019-06-30')

data_folder = join(const_rat.base_save_folder, const_rat.rat_folder, const_rat.date_folders[date_folder], 'Data')
...
conversion_const = const_comm.PIXEL_PER_FRAME_TO_CM_PER_SECOND
...
speed_very_corr_neurons_index = 663
speed_very_corr_neurons = template_info.loc[int(speed_very_corr_neurons_index)]

brain_positions_of_corr_neurons = speed_very_corr_neurons['position Y'] * const_comm.POSITION_MULT
...
brain_positions_of_corr_neurons = np.array([speed_corr_neurons['position X'].values * const_comm.POSITION_MULT,
                                            speed_corr_neurons['position Y'].values * const_comm.POSITION_MULT])

spp.view_grouped_templates_positions(kilosort_folder, const_comm.BRAIN_REGIONS, const_comm.PROBE_DIMENSIONS,
                                     const_comm.POSITION_MULT, template_info=speed_corr_neurons,
...
for touch_ball in time_points_of_touch_ball:
    if time_points_of_trial_pokes[index_p] - touch_ball < 40 * const_comm.SAMPLING_FREQUENCY:
...
window_size_frames = np.array(trial_durations / const_comm.SAMPLING_FREQUENCY * 120, dtype=np.int32)
...
#speed_corr_neurons_index = np.squeeze(np.argwhere(mutual_infos_spikes_vs_speed > mean_sh+confi_intervals[1]))
speed_corr_neurons_index = np.squeeze(np.argwhere(mutual_infos_spikes_vs_speed >0.016))
speed_corr_neurons = template_info.loc[speed_corr_neurons_index]
