...
lfps = ns_funcs.load_binary_amplifier_data((lfps_file), number_of_channels=const_comm.NUMBER_OF_LFP_CHANNELS_IN_BINARY_FILE)

imfs_file = join(const_rat.base_save_folder, const_rat.rat_folder, const_rat.date_folders[date_folder],
...
spike_times_subsampled = (spike_times / const_comm.LFP_DOWNSAMPLE_FACTOR).astype(np.int)

spike_triggered_lfp_avg_shape = (len(spike_times), const_comm.NUMBER_OF_LFP_CHANNELS_IN_BINARY_FILE, 2 * half_time_window)
...
for neuron in np.arange(len(spike_times)):
    ...
    for channel in np.arange(const_comm.NUMBER_OF_LFP_CHANNELS_IN_BINARY_FILE):
