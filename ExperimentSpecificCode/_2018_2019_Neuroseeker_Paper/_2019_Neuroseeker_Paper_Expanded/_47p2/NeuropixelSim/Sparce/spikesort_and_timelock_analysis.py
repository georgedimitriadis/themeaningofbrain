...
y_positions_sorted = y_positions[position_sorted_indices] * const_com.POSITION_MULT


neuropixel_electrodes = 384
neuropixel_height = neuropixel_electrodes / 2 * 22.4

from_top_of_cortex = const_rat.BRAIN_REGIONS['Cortex MPA'] - neuropixel_height
cell_indices_from_cortex = position_sorted_indices[y_positions_sorted > from_top_of_cortex]
frs_norm_from_cortex = tn[cell_indices_from_cortex]

plt.figure(1)
plt.imshow(np.flipud(frs_norm_from_cortex), aspect='auto', extent=[-8, 8, len(frs_norm_from_cortex), 0])
plt.hlines(y=len(t) - region_lines, xmin=-8, xmax=8, linewidth=2, color='w')
plt.vlines(x=0, ymin=0, ymax=len(frs_norm_from_cortex) - 1)


from_hippocampus = const_rat.BRAIN_REGIONS['CA1'] - neuropixel_height
cell_indices_from_hippocampus = position_sorted_indices[np.logical_and(y_positions_sorted > from_top_of_cortex,
                                                                       y_positions_sorted < const_rat.BRAIN_REGIONS['CA1'])]
frs_norm_from_hippocampus = tn[cell_indices_from_hippocampus]
first_hippocampal_cell_index = len(position_sorted_indices[y_positions_sorted > const_rat.BRAIN_REGIONS['CA1']])

plt.figure(2)
plt.imshow(np.flipud(frs_norm_from_hippocampus), aspect='auto', extent=[-8, 8, len(frs_norm_from_hippocampus) + first_hippocampal_cell_index, first_hippocampal_cell_index])
plt.hlines(y=len(t) - region_lines, xmin=-8, xmax=8, linewidth=2, color='w')
plt.vlines(x=0, ymin=first_hippocampal_cell_index, ymax=len(frs_norm_from_hippocampus) + first_hippocampal_cell_index)
...
np.save(join(analysis_folder, 'Results', 'MutualInformation',
             'mutual_infos_NPsparceProbe_spike_rates_vs_spike_rates_all_neurons_around_succ_trials.npy'), mutual_infos)
