...
# oldCoord=np.genfromtxt(r'C:\Users\Kampff\Downloads\coordinatetransformation\oldCoordinates3.dat', delimiter=',')
oldCoord = np.genfromtxt(r'F:\softwareCorrection\OLD_REAL.dat',
                         delimiter=',')
# newCoord=np.genfromtxt(r'C:\Users\Kampff\Downloads\coordinatetransformation\newCoordinates3.dat', delimiter=',')
newCoord = np.genfromtxt(r'F:\softwareCorrection\NEW_REFERENCE.dat',
                         delimiter=',')
 newCoordErrors=np.genfromtxt(r'F:\softwareCorrection\newCoordinates_errors.dat', delimiter=',')



#newCoordErrors = np.genfromtxt(
#    r'E:\Paper\correcao software\coordinatetransformation\newCoordinates_errors_combined.dat', delimiter=',')
...
#oldCoord = np.genfromtxt(r'E:\Paper\correcao software\oldCoordinates2.dat', delimiter=',')
#newCoord = np.genfromtxt(r'E:\Paper\correcao software\newCoordinates2.dat', delimiter=',')
#newCoordErrors = np.genfromtxt(r'E:\Paper\correcao software\newcoordinates_errors.dat', delimiter=',')
...
for i in range(n_points):
    blah = (matrix[0][0] * oldCoord[i][0] + matrix[0][1] * oldCoord[i][1] + matrix[0][2] * oldCoord[i][2] - newCoord[i][
        0]) ** 2
    blah = blah + (matrix[1][0] * oldCoord[i][0] + matrix[1][1] * oldCoord[i][1] + matrix[1][2] * oldCoord[i][2] -
                   newCoord[i][1]) ** 2
    blah = blah + (matrix[2][0] * oldCoord[i][0] + matrix[2][1] * oldCoord[i][1] + matrix[2][2] * oldCoord[i][2] -
                   newCoord[i][2]) ** 2
...
A = np.array([(7.36412650e-02, 1.25686386e-03, 6.61695851e-02), (-1.68983025e-04, 1.01548734e-01, -1.14021148e-03),
              (6.77851468e-02, -8.28774803e-04, -7.44689774e-02)])
b = np.array([-255.5548, 89.5, -2238.886])
