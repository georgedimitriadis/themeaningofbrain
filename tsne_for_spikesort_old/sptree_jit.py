...
class SPTree:
    ...
    def compute_non_edge_forces(self, point_index, theta, neg_force, sum_q):
        compute_non_edge_forces(self, point_index, theta, neg_force, sum_q)

    def compute_edge_forces(self, indices_p, values_p, N):
        return compute_edge_forces(self.data, self.dimension, indices_p, values_p, N)


@jit
def compute_non_edge_forces(tree, point_index, theta, neg_force, sum_q):
    # Make sure that we spend no time on empty nodes or tree-interactions
    if tree.cum_size == 0 or (tree.is_leaf and tree.size == 1 and tree.index[0] == point_index):
        return

    # Compute distance between point and center-of-mass
    buffer = tree.data[point_index, :] - tree.center_of_mass
    distance = np.sum(buffer * buffer)

    # Check whether we can use this node as a "summary"
    max_width = np.max(tree.boundary.get_width())
    if tree.is_leaf or max_width / np.sqrt(distance) < theta:
        # Compute and add t-SNE force between point and current node
        distance = 1 / (1 + distance)
        mult = tree.cum_size * distance
        sum_q[0] += mult
        mult *= distance
        neg_force[point_index, :] += mult * buffer
    else:
        # Recursively apply Barnes-Hut to children
        for i in np.arange(tree.num_children):
            tree.children[i].compute_non_edge_forces(point_index, theta, neg_force, sum_q)


@jit
def compute_edge_forces(data, dimension, indices_p, values_p, N):
    pos_force = np.zeros((N, dimension))

    for n in np.arange(N):
        distances = np.ones(indices_p.shape[1])
        a = data[indices_p[n, :], :]  # shape = (per*3 , dimension)
        b = data[n, :]
        buffer = a - b # shape = (per*3, dimension)
        distances += np.sum(buffer**2, axis=1)  # shape = (per*3)
        distances = values_p[n, :] / distances
        pos_force[n, :] += np.sum(np.tile(distances, (dimension, 1)).T * buffer, axis=0)  # shape = (dimension)
