...
def _cube_of_spikes(memmap_filename,
                    spike_times,
                    num_of_channels,
                    cut_time_points_around_spike,
                    number_of_timepoints_in_raw,
    ...
    num_of_spikes = np.size(spike_times)
    y = np.zeros((num_of_channels, cut_time_points_around_spike * 2))
    # remove any spikes that don't have enough time points
    too_early_spikes = np.squeeze(np.argwhere(spike_times < cut_time_points_around_spike), axis=1)
    too_late_spikes = np.squeeze(
        np.argwhere(spike_times > number_of_timepoints_in_raw - cut_time_points_around_spike), axis=1)
    out_of_time_spikes = np.concatenate((too_early_spikes, too_late_spikes))

    spike_times = np.delete(spike_times, out_of_time_spikes)
    num_of_spikes = spike_times.shape[0]

    for spike_time in spike_times:
        y = data_raw_matrix[active_channel_map,
                spike_time - cut_time_points_around_spike:
                spike_time + cut_time_points_around_spike]
