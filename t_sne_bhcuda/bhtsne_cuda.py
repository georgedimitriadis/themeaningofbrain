...
def t_sne(samples, use_scikit=False, files_dir=None, results_filename='result.dat', data_filename='data.dat',
    ...
    if use_scikit:  # using python's scikit tsne implementation
        ...
        if results_filename is not 'result.dat':
            replace(path_join(files_dir, 'result.dat'), path_join(files_dir, results_filename))
...
def save_data_for_tsne(samples, files_dir, filename, theta, perplexity, eta, no_dims, iterations, seed, gpu_mem,
    ...
    filename: The name of the file that the t_sne_bhcuda.exe looks into for data to t-sne. This data file also has a header with all the parameters that the t_sne_bhcuda.exe needs to run.
    theta: If > 0 then the algorithm run the burnes hat aproximation (with angle = theta). If = 0 then it runs the
     exact version. Values smaller than 0.5 do not add to much error.
    perplexity: Defines the amount of samples whose distances are comparred to every sample (check sklearn and the van der Maatens paper)
    eta: The learning rate
    no_dims: Number of dimensions of the t-sne embedding
    iterations: The number of itterations (usually around 1000 should suffice)
    seed: Set to a number > 0 if the amount of samples is too large to t-sne
    gpu_mem : If > 0 (and <= 1) then the t_sne_bhcuda.exe will run the eucledian distances calculations on the GPU (if possible) and will use (gpu_mem * 100) per cent of the available gpu memory to temporarily store results. If = 0 then the t_sne_bhcuda.exe will run only on the CPU.
    verbose: Define verbosity. 0 = No output, 1 = Basic output, 2 = Full output, 3 = Also save t-sne results in interim files after every iteration. Option 3 is used to save all steps of t-sne to explore the way the algorithm seperates the data (good for movies).
    randseed: Set the random seed for the initiallization of the samples on the no_dims plane.
